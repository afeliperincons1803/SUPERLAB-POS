...
function showCommandTicket(order) {
  openModal(`<div class="thermal-preview"><div class="thermal-ticket command-ticket" data-ticket="command"><div class="command-ticket-head"><strong>COMANDA · ${preparationName(order.preparation_status).toUpperCase()}</strong><h2>${escapeHtml(order.number)}</h2><div class="call-name">LLAMAR A<br><b>${escapeHtml(order.customer_name||'SIN NOMBRE')}</b></div><small>${dateFmt(order.created_at)} · ${escapeHtml(order.source==='tablet'?'Tablet':'Caja')}</small></div><div class="command-ticket-items">${order.items.map(x=>`<section><h3>${x.quantity} × ${escapeHtml(x.name)}</h3>${x.toppings?`<p>${escapeHtml(x.toppings)}</p>`:''}${Number(x.discount_percent)>0?`<small>Descuento ${x.discount_percent}%</small>`:''}</section>`).join('')}</div>${order.notes?`<div class="command-ticket-note"><strong>NOTA GENERAL</strong><p>${escapeHtml(order.notes)}</p></div>`:''}<footer>Preparar y marcar como lista en el POS</footer></div>${printerPreviewPanel('command')}</div><div class="form-actions"><button class="button secondary" data-close-modal>Cerrar</button><button class="button secondary" id="print-command">Impresión del navegador</button></div>`);
  $('#print-command').onclick=()=>printThermal('command');
  setupPrinterPreview(order,'command');
}
...
function setupPrinterPreview(order,kind){
...
async function loadOrders() {
  ...
  $('#orders-table').innerHTML=orders.length?orders.map((o,i)=>`<tr><td><strong>${o.number}</strong><br><small>${escapeHtml(o.customer_name||'Sin nombre')} · ${o.source==='tablet'?'Autoservicio':'Caja'} · ${o.items.length} líneas</small></td><td>${dateFmt(o.created_at)}</td><td>${escapeHtml(o.cashier)}</td><td><span class="status-badge ${o.preparation_status}">${preparationName(o.preparation_status)}</span></td><td>${paymentName(o.payment_method)}</td><td><strong>${fmt(o.total)}</strong></td><td><div class="row-actions">${o.status==='paid'?`<button data-receipt="${i}">Factura</button>`:''}<button data-view-command="${i}">Comanda</button></div></td></tr>`).join(''):`<tr><td colspan="7">Todavía no hay pedidos.</td></tr>`;
