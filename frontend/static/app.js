...
function canUseFormulaX(product){return new Set(['001','002','003','004','005','006','007','008','009','017']).has(String(product.sku||''))}
function customizationSchema(product) {
  const sku=String(product.sku||'');
  const fruits=toppingsByGroup('Frutas'), sauces=toppingsByGroup('Salsas'), freeAdds=toppingsByGroup('Adicionales sin costo'), sweets=[...toppingsByGroup('Dulces'),...toppingsByGroup('Crunch'),...toppingsByGroup('Perlas')];
  const schemas={
    ...
    '010':[{title:'Elige 5 frutas',type:'multi',max:5,min:5,options:fruits},{title:'Elige la base',type:'single',min:1,options:['Yogur','Crema de Leche','Chamoy']},{title:'Elige 3 toppings',type:'multi',max:3,min:3,options:sweets},{title:'Elige hasta 2 salsas',type:'multi',max:2,min:1,options:sauces},{title:'Adicionales sin costo',type:'multi',max:3,min:0,options:freeAdds}],
    '011':[{title:'Elige 4 frutas premium',type:'multi',max:4,min:4,options:fruits},{title:'Elige hasta 3 toppings y gomitas',type:'multi',max:3,min:1,options:sweets},{title:'Elige hasta 2 salsas',type:'multi',max:2,min:1,options:sauces},{title:'Adicionales sin costo',type:'multi',max:3,min:0,options:freeAdds}],
    '015':[{title:'Elige 3 toppings',type:'multi',max:3,min:3,options:sweets},{title:'Elige 2 salsas',type:'multi',max:2,min:2,options:sauces}],
    '016':[{title:'Elige 5 toppings',type:'multi',max:5,min:5,options:sweets},{title:'Elige 3 salsas',type:'multi',max:3,min:3,options:sauces}],
...
function showProductCustomizer(product) {
  ...
  const formulaSection=canUseFormulaX(product)?`<section class="custom-step"><h3>Fórmula X <small>potenciador opcional solo para bebidas</small></h3><div class="choice-grid"><label class="choice-pill"><input type="radio" name="booster" value="" checked><span>Sin Fórmula X</span></label>${POTENCIADORES_LAB.map(x=>`<label class="choice-pill paid"><input type="radio" name="booster" value="${escapeAttr(x.code)}" data-label="${escapeAttr(x.name)}" data-price="${x.price}"><span>${escapeHtml(x.name)}<br><small>+ ${fmt(x.price)}</small></span></label>`).join('')}</div><div id="booster-flavor"></div></section>`:'';
  openModal(`<div class="customizer"><p class="eyebrow orange">PREPARA EL PEDIDO</p><h2>${escapeHtml(product.name)}</h2><p class="muted">${escapeHtml(product.description||'')}</p><div class="custom-total"><span>Base ${fmt(product.price)}</span><strong id="custom-total">${fmt(product.price)}</strong></div><form id="custom-form">${schema.map((section,i)=>customSection(section,i)).join('')}${formulaSection}<label class="custom-note">Nota para preparación<input name="note" placeholder="Ej. sin picante, más hielo, separar salsa…"></label><div class="form-actions"><button type="button" class="button secondary" onclick="document.querySelector('.modal-close').click()">Cancelar</button><button class="button primary">Agregar al pedido</button></div></form></div>`);
